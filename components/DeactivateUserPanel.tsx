...
interface DeactivateUserPanelProps {
  isOpen: boolean;
...
/**
 * Deactivate flow for ASE / ASM / SM. Picker source is hierarchy.siblings
 * (peers in the same RBL domain), enforcing the "domain integrity" rule:
 * a user's responsibilities can only transfer to a peer in their domain.
 */
export const DeactivateUserPanel: React.FC<DeactivateUserPanelProps> = ({ isOpen, user, onClose, onSuccess }) => {
  const { showToast } = useToast();
  const invalidateHierarchy = useInvalidateHierarchy();

  const isAse = user?.role === 'ASE';
  ...
  const selectedSuccessor = useMemo(() => peers.find(s => s.id === successorId) || null, [peers, successorId]);

  const noPeers = !hierarchyLoading && peers.length === 0;
  const canSubmit = Boolean(user && successorId && reason.trim().length >= 4 && !submitting && !noPeers);
  ...
  const handleSubmit = async () => {
    ...
    setSubmitError(null);
    try {
      const payload: Parameters<typeof apiService.users.deactivate>[1] = {
        reason: reason.trim(),
      };
      if (isAse) {
      ...
      const res = await apiService.users.deactivate(user.id, payload);
      if (res.success) {
        showToast(`${user.name || 'User'} deactivated`, 'success');
        invalidateHierarchy();
        onSuccess();
        onClose();
      } else {
        setSubmitError(res.error || 'Failed to deactivate user.');
      }
    } catch (err) {
  ...
  const transferLabel = isAse ? 'Successor ASE' : `Transfer subordinates to (another ${roleLabel})`;
  const transferHelp = isAse
    ? `All outlets and distributors mapped to ${user.name} will move to the selected ASE.`
    : `All direct reports of ${user.name} will move to the selected ${roleLabel}.`;
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              disabled={!canSubmit}
              className="px-4 py-2 text-sm font-bold text-white bg-red-600 hover:bg-red-700 rounded-xl shadow-sm shadow-red-500/25 transition-colors disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-2"
            >
              {submitting && (
    ...
    >
      <div onKeyDown={handleKeyDown} className="space-y-7">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <span className="text-[10px] font-bold text-red-500 uppercase tracking-[0.15em]">User loses access immediately</span>
        </div>
        ...
        {!hierarchyLoading && noPeers && (
          <div className="p-4 text-sm text-amber-700 bg-amber-50 border border-amber-100 rounded-xl">
            No peer <strong>{roleLabel}</strong> exists in this domain to take over. Add a peer first, then come back.
          </div>
        )}
        ...
        {!hierarchyLoading && !noPeers && (
          <>
            <section>
              ...
              <p className="text-xs text-slate-400 mb-2">{transferHelp}</p>
            ...
            <section>
              <label htmlFor="deactivate-reason" className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2">
                Reason
              </label>
              <textarea
                ...
                rows={3}
                placeholder="e.g. Resigned effective 2026-05-31"
                className="w-full p-3 bg-white border border-slate-200 rounded-xl text-sm font-medium text-slate-700 placeholder:text-slate-400 focus:ring-2 focus:ring-red-500/20 focus:border-red-400 focus:outline-none transition-all resize-none"
              />
            </section>

            {selectedSuccessor && (
              <section className="p-4 rounded-xl bg-red-50/40 border border-red-100">
                <p className="text-[10px] font-black text-red-500 uppercase tracking-[0.2em] mb-2">Review</p>
                <p className="text-sm text-slate-700 leading-relaxed">
                  <span className="font-bold">{user.name}</span> will be marked <span className="font-bold">INACTIVE</span> and lose access immediately.{' '}
                  {isAse
